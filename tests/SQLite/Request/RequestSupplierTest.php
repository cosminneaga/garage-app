...
test('super: only super should directly update supplier details', function () {
    $this->user->givePermissionTo([
        Permission::value(UserPermission::SUPPLIER, 'update'),
    ]);

    actingAs($this->super);
    put(route('super.suppliers.update', [$this->supplier, $this->company]), [
        'name' => 'Supplier',
        'code' => 'SUP',
        'type' => SupplierType::DISTRIBUTOR->value,
        'tax_id' => '39287398283782',
        ...
        ->assertSessionHas('message', (object) [
            'type' => 'success',
            'title' => 'Supplier updated',
            'message' => 'Supplier information has been successfully updated',
        ]);
...
test('super: only super role should directly destroy supplier', function () {
    $this->user->givePermissionTo([
        Permission::value(UserPermission::SUPPLIER, 'delete'),
    ]);

    actingAs($this->super);
    delete(route('super.suppliers.destroy', $this->supplier))
        ->assertRedirectBack()
        ->assertSessionHas('message', (object) [
            'type' => 'info',
            'title' => 'Supplier removed',
            'message' => 'Supplier has been successfully removed',
        ]);
...
test('super: only super role should directly restore supplier', function () {
    $this->supplier->delete();
    $this->user->givePermissionTo([
        Permission::value(UserPermission::SUPPLIER, 'restore'),
    ]);

    actingAs($this->super);
    post(route('super.suppliers.restore', $this->supplier))
        ->assertRedirectBack()
        ->assertSessionHas('message', (object) [
            'type' => 'success',
            'title' => 'Supplier restored',
            'message' => 'The supplier has been successfully restored',
        ]);
